...
-- // Tables
local Sanity, Visuals, Color, Math = {
    Safe = false,
    Connections = {},
}, {
    Drawings = {},
...
local Settings = {
    ...
    MaxDistance = 750,
    FadeTime = 2,
    TextCase = "lowercase", -- {"UPPERCASE", "Normal", "lowercase"}
    TextLength = 24,
    Box = {false, "Corner", NewHex("#ffffff"), NewHex("#000000")},
...
--
do -- // Sanity
    function Sanity:ThreadFunction(Func, Name, ...)
        local Func = Name and function()
            local Passed, Statement = pcall(Func)
            --
            if not Passed and not Sanity.Safe then
                warn("Sanity:\n", "              " .. Name .. ":", Statement)
            end
        end or Func
    ...
    --
    function Sanity:Connection(Type, Callback)
        local Connection = Type:Connect(Callback)
        Sanity.Connections[#Sanity.Connections + 1] = Connection
        --
        return Connection
    end
    --
    function Sanity:GetBodyParts(Character, RootPart, Indexes, Hitboxes)
        local Parts = {}
    ...
    --
    function Sanity:GetPlayerParent(Player)
        return Player.Parent
    end
    --
    function Sanity:GetCharacter(Player)
        return Player.Character
    end
    --
    function Sanity:GetHumanoid(Player, Character)
        return Character:FindFirstChildOfClass("Humanoid")
    end
    --
    function Sanity:GetRootPart(Player, Character, Humanoid)
        return Humanoid.RootPart
    end
    --
    function Sanity:GetHealth(Player, Character, Humanoid)
        if Humanoid then
            return Clamp(Humanoid.Health, 0, Humanoid.MaxHealth), Humanoid.MaxHealth
        end
    end
    --
    function Sanity:ValidateClient(Player)
        local Object = Sanity:GetCharacter(Player)
        local Humanoid = (Object and Sanity:GetHumanoid(Player, Object))
        local RootPart = (Humanoid and Sanity:GetRootPart(Player, Object, Humanoid))
        --
        return Object, Humanoid, RootPart
    end
    --
    function Sanity:ClientAlive(Player, Character, Humanoid)
        local Health, MaxHealth = Sanity:GetHealth(Player, Character, Humanoid)
        --
        return (Health > 0)
    end
    --
    function Sanity:ClampString(String, Length, Font)
...
do -- // Visuals
    ...
    function Visuals.Base:Update()
        ...
        if Self then
            ...
            local Parent = Sanity:GetPlayerParent(Player)
            --
            if (Player and Parent and Parent ~= nil) or (Info.RootPosition and Info.Health and Info.MaxHealth) then
                if (Settings.Enabled == true) then
                    local Object, Humanoid, RootPart = Sanity:ValidateClient(Player)
                    local BodyParts = (RootPart and Sanity:GetBodyParts(Object, RootPart, true))
                    local TransparencyMultplier = 1
                    --
                    if Object and Object.Parent and (Humanoid and RootPart and BodyParts) then
                        local Health, MaxHealth = Sanity:GetHealth(Player, Object, Humanoid)
                        --
                        if Health > 0 then
                            Info.Pass = true
                                ...
                                if (Settings["HealthBar"][1] == true) then
                                    ...
                                    do -- // Value
                                        if (Settings["HealthNumber"][1] == true) then
                                            local Text = Sanity:ClampString(tostring(Round(Info.Health)), BoxSize.Y)
...
--
Sanity:Connection(RunService.RenderStepped, function(Delta)
    for Index, Base in pairs(Visuals.Bases) do
        Sanity:ThreadFunction(function()
            Base:Update()
        end, "3x01")
    end 
end)
--
Sanity:Connection(Players.PlayerAdded, function(Player)
